...
function Arm({ side, angleDeg, color, tool = false }: ArmProps) {
  const shoulderX = side === "left" ? 32 : 68;
  const dir = side === "left" ? -1 : 1;
  const handX = shoulderX + dir * 3;
  const handY = 92;
  return (
    <g
      style={{
    ...
    >
      <line x1={shoulderX} y1={48} x2={handX} y2={handY} stroke={color} strokeWidth={5} strokeLinecap="round" />
      <circle cx={handX} cy={handY} r={5} fill={color} />
      {/* Drawn inside this same rotating group, at the hand's local
          coordinate, so it actually tracks the hand through arm rotation
          instead of sitting at a fixed point independent of the pose. */}
      {tool && (
        <g transform={`translate(${handX}, ${handY}) rotate(20)`}>
          <rect x={-2} y={-16} width={4} height={16} rx={2} fill={color} />
...
export function Character({ role, leftArmAngle = 0, rightArmAngle = 0, holdingTool = false, className }: CharacterProps) {
  ...
  return (
    <svg
      ...
      className={className}
      aria-hidden
      style={{ animation: "marbury-idle-bob 2.6s ease-in-out infinite" }}
    >
      {/* legs */}
      ...
      <Arm side="right" angleDeg={rightArmAngle} color={COLORS.outline} tool={holdingTool} />
